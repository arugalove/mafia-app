...
export const filterPlayersByFaction = (
  players: Player[],
  faction: Faction
): Player[] => {
...
export const checkWinCondition = (gameState: GameState): GameState => {
  ...
  if (numberOfVillagersAlive <= numberOfMafiaAlive)
    return { ...gameState, winner: Faction.Mafia };
  if (numberOfMafiaAlive === 0)
    return { ...gameState, winner: Faction.Village };
  return gameState;
};
